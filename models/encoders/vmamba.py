# 核心作用: VMamba (VSSM) 的底层实现。这个文件包含了Vision Mamba模型的核心算法和模块定义，是整个项目能够运行的基础。
# CUDA算子封装: 通过torch.autograd.Function封装了底层的selective_scan_cuda算子 ，这是Mamba实现高性能的关键。
# 2D扫描逻辑: 实现了CrossScan和CrossMerge ，它们将2D图像数据展平为多个1D序列，以适应Mamba的扫描机制，这是Vision Mamba的核心思想。
# SS2D模块: 这是2D状态空间模型的核心模块 ，它整合了卷积、线性投影和核心的
# selective_scan操作。
# VSSBlock: 将SS2D模块与残差连接、归一化层和MLP层组合成一个完整的block ，类似于Transformer中的Block。
# VSSM & Backbone_VSSM: 将多个VSSBlock和下采样层堆叠起来，构建了一个完整的、分层的视觉骨干网络（Backbone）。
...
class Backbone_VSSM(VSSM):
    def __init__(self,
                 patch_size=4,          #补丁大小
                 in_chans=3,            #输入通道数，对应RGB彩色图像
                 num_classes=1000,      #最终分类头的输出类别数。默认值1000是ImageNet数据集的标准。对于分割任务，这个分类头会被忽略或删除。
                 depths=[2, 2, 9, 2],   #一个列表，定义了网络4个阶段（Stage）的深度，即每个阶段分别包含2, 2, 9, 2个 VSSBlock。
                 dims=[96, 192, 384, 768],      #定义了网络4个阶段的特征维度（通道数）
                 d_state=16,        #Mamba核心状态空间模型（SSM）的状态维度 N
                 ssm_ratio=2.0,     #VSSBlock内部的扩展比例，用于控制SSM部分的计算维度。
                 attn_drop_rate=0., #自注意力层中的dropout概率。Dropout的比率，用于防止过拟合，0.表示不使用。
                 drop_rate=0.,
                 drop_path_rate=0.1,    #线性层中的dropout概率。Dropout的比率，用于防止过拟合，0.表示不使用。
                 mlp_ratio=4.0,         #VSSBlock内部MLP层的扩展比例。
                 patch_norm=True,       #是否在 PatchEmbed 之后添加一个归一化层。
                 norm_layer=nn.LayerNorm,   # 指定整个网络使用的归一化层类型。
                 downsample_version: str = "v2",        #指定连接不同阶段的下采样层所使用的版本。
                 use_checkpoint=False,      #是否使用梯度检查点（Gradient Checkpointing）。这是一种用计算时间换取显存的技术，在模型非常大、显存不足时可以开启。
                 out_indices=(0, 1, 2, 3),  #它告诉骨干网络，在前向传播结束后，需要返回哪几个阶段的输出特征图。(0, 1, 2, 3) 表示需要返回全部4个阶段的输出。这对于需要多尺度特征的下游任务（如分割、检测）至关重要。
                 pretrained=None,
                 **kwargs,
        ):
        ...
        for i in out_indices:
            layer = norm_layer(self.dims[i])            #在循环的每一次中，动态地创建一个归一化层。self.dims 是一个包含各阶段通道数的列表
            layer_name = f'outnorm{i}'                  #为刚刚创建的归一化层生成一个独一无二的名字。
            self.add_module(layer_name, layer)          #这是PyTorch nn.Module 的一个标准方法，用于将一个模块（在这里是 layer）作为子模块添加到当前模型中。

        del self.classifier     #删除分类头, 因为这个模型不需要分类头
        self.load_pretrained(pretrained)        #加载 pretrained 参数所指定的预训练权重。

    def load_pretrained(self, ckpt=None, key="model"
                                             ""):
        #检查您是否提供了一个 .pth 权重文件的路径 (ckpt)。
        if ckpt is None:
            return
        ...
        try:
            ...
            # 告诉PyTorch：“请尽量加载所有能匹配上的权重。如果遇到预训练权重里有，但我当前模型里没有的层（比如分类头），就忽略它
    ...
    def forward(self, x):
        ...
        #这是在 __init__ 中定义的 Stem 模块,进行“Patch Embedding”操作（即分块和特征嵌入）将高分辨率、低通道的图像转换为低分辨率、高通道的初始特征图
        x = self.patch_embed(x)
        outs = []           # 这个列表将用来收集并存储编码器在不同深度阶段（Stage）输出的特征图。

        #含义: 开始一个循环，遍历模型的所有阶段。self.layers 应该是一个包含了模型4个阶段处理模块的列表。
        for i, layer in enumerate(self.layers):     #这个函数接收当前阶段的处理层 layer 和上一阶段的输出 x 作为输入。

            #调用一个名为 layer_forward 的辅助函数（这个函数可能定义在文件的其他地方）。这个函数接收当前阶段的处理层 layer 和上一阶段的输出 x 作为输入。
            # 这种设计将“本阶段处理”和“为下一阶段准备输入”这两个步骤清晰地分离开，使得循环体内部的逻辑非常干净。
            o, x = layer_forward(layer, x) # (B, H, W, C)
            #x经过layer（包含一系列的VSSBlock）后得到输出得到o, 将o进行下采样赋值给x
            #保留o的作用是为解码器提供”跳跃连接“

            if i in self.out_indices:
                #获取为当前阶段准备的专属归一化层（如 self.outnorm0）。
                norm_layer = getattr(self, f'outnorm{i}')
                #out = norm_layer(o): 关键点！ 对 o （也就是未经下采样的当前阶段输出）进行归一化处理。
                out = norm_layer(o)
                #out = out.permute(0, 3, 1, 2).contiguous(): 将数据格式从 channel-last ([B, H, W, C]) 转换为 PyTorch 标准的 channel-first ([B, C, H, W])。
                out = out.permute(0, 3, 1, 2).contiguous()
                # 将处理好的特征图 out 添加到 outs 列表中，等待最后返回给解码器。
                outs.append(out)
        #这是一个特殊情况的处理。如果 out_indices 是一个空列表，意味着我们不需要任何中间阶段的特征图（例如，在纯粹的图像分类任务中）。
