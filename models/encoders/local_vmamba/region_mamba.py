# 定义并实现双通道交互模块 (DCIM)。
# 这个文件是模型进行多模态特征融合（PET与CT）的核心。它设计了一种基于“区域-全局”思想的Mamba模块，让两个模态的特征在不同的尺度上进行信息交互。

from einops import rearrange, repeat            #这是一个非常强大的张量操作库，可以让复杂的 reshape, permute 等操作变得更简洁易读。
import math
import torch
...
from timm.models.layers import DropPath, to_2tuple, trunc_normal_           #从 timm 库导入一些常用的神经网络层，如 DropPath (随机深度)、to_2tuple (将单个数字转为元组)等。
from ..vmamba import SS2D,cross_selective_scan,SelectiveScan,CrossMerge,CrossScan           #从父目录的 vmamba.py 文件中导入Mamba的核心组件，如 SS2D。
try:
...
# x和y（分别代表PET和CT的特征）分解为“内部Token”（inner_tokens）和“外部Token”（outer_tokens） 。
# 你可以把它理解为一种“图像分块”操作。它首先将特征图看作是由多个大区域（Region/Patch）组成的，outer_tokens就是每个大区域的概括性表示 。
# 每个大区域内部又由更小的“视觉词”（Visual Word）组成，inner_tokens就代表了这些更细粒度的内部信息
#Stem 类的作用是进行特征分词 (Tokenization)，即将输入的PET和CT特征图分解为两种不同粒度的“令牌”（Token），为后续的 Region_global_Block 做准备。
class Stem(nn.Module):

    def __init__(self,  outer_dim=768, inner_dim=24):           #定义了外部（区域）和内部（局部）令牌的特征维度。
        super().__init__()
        self.inner_dim=inner_dim
        self.inner_convs = nn.Sequential(               # 一系列卷积层，用于处理输入x（将被分解为内部令牌）。
            nn.Conv2d(inner_dim, inner_dim//4, 3, stride=1, padding=1),
            nn.BatchNorm2d(inner_dim//4),           #批归一化
            nn.ReLU(inplace=True),
            nn.Conv2d(inner_dim//4 , inner_dim, 3, stride=1, padding=1),
            nn.BatchNorm2d(inner_dim),
            nn.ReLU(inplace=False),
        )
        self.outer_convs = nn.Sequential(               #一系列卷积层，用于处理输入y（将被分解为外部令牌）。
            nn.Conv2d(inner_dim , inner_dim//16, 3, stride=1, padding=1),
            nn.BatchNorm2d(inner_dim //16),
            nn.ReLU(inplace=True),
            nn.Conv2d(inner_dim//16, inner_dim // 8, 3, stride=2, padding=1),           #stride=2 的卷积层起到了下采样的作用。
            nn.BatchNorm2d(inner_dim // 8),
            nn.ReLU(inplace=True),
        ...
        self.kern=4             #self.kern=4: 定义了分块的核心尺寸，即每个大区域（Patch）的大小是 4x4。
        self.unfold = nn.Unfold(kernel_size=self.kern, padding=0, stride=self.kern)

    #forward: 前向传播函数，接收两个输入 x（CT） 和 y (PET)。
    def forward(self, x,y):
        B, C, H, W = x.shape

        #H_out, W_out: 计算出特征图可以被分割成多少个Patch, H_out = H // 4
        H_out, W_out = H // self.kern, W // self.kern
        #H_in, W_in: 定义内部Patch的大小
        H_in, W_in = self.kern, self.kern  # Every visual sentence is composed of 4x4 visual words
        w=self.kern
        #定义特征图被分成几个Patch
        Hg, Wg = math.ceil(H /  H_in), math.ceil(W / H_in)          #math.ceil 确保即使不能整除也能覆盖所有区域。
        # inner_tokens生成内部令牌（Local tokens）。
        inner_tokens = self.inner_convs(x)  # B, C, H, W

        #然后通过一系列复杂的 view, permute, reshape 操作，将特征图 x 重新排列组合。这里的核心思想是将每个 4x4 的小块区域内的特征拉成一个序列。
        ...
        #将ct特征图分解为batch为B*N的长度为16的一维张量，序列中的每一个元素的通道数为H_in * W_in
        inner_tokens =inner_tokens.view(B * H_out * W_out, self.inner_dim, H_in * W_in).transpose(1,2)

        #outer_tokens: 生成外部令牌（Region tokens）。通过 outer_convs 对输入 y 进行特征提取和下采样。
        outer_tokens = self.outer_convs(y)  # B, C, H_out, W_out
        #将PET特征图分解一个batch为4的一维张量，长度为H_out * W_out，通道数自动推断
        #permute 和 reshape 操作将输出的特征图拉平成一个序列。最终 outer_tokens 的形状是 (B, N, C_outer)，其中 N 是大区域的数量，每个令牌代表一个大区域的概括性信息。
...
#DCIM模块 ：负责并行处理PET的“区域令牌”和CT的“局部令牌”，并执行最关键的动态信息交互。
class Region_global_Block(nn.Module):
    ...
    def __init__(self, outer_dim, inner_dim,  num_words,
        ...
        # Inner
        self.inner_dim = inner_dim          #实例化一个 SS2D_Region 模块，用于处理 inner_tokens（局部细化）。
        self.outer_dim =outer_dim           #实例化另一个 SS2D_Region 模块，用于处理 outer_tokens（全局交互）。

        # 定义归一化层：用于处理内部令牌（inner_tokens）
        self.inner_norm1 = norm_layer(num_words * inner_dim)        #将一个区域内的所有局部令牌（num_words个，每个维度为inner_dim）展平后，进行整体归一化。

        # 局部细化: 它使用另一个SS2D_Region模块（self.inner_attn）对inner_tokens进行处理 。这一步是在每个大区域
        # 内部独立进行的，目的是细化和增强每个区域内的局部特征 。
        self.inner_attn =  SS2D_Region(d_model=inner_dim, dropout=0, d_state=16)

        self.kern=4

        self.outer_norm1 = norm_layer(outer_dim)           # 创建另一个归一化层，这个专门用于处理外部令牌
        # 全局交互: 它使用一个SS2D_Region模块（self.outer_attn）对outer_tokens进行处理 。这一步的目的是让代表不同区域的outer_tokens进行全局信息交换。
        # 因为outer_tokens融合了两个模态的信息，所以这一步实现了跨模态的全局上下文交互 。
        self.outer_attn =  SS2D_Region(d_model=outer_dim, dropout=0, d_state=16)
        #创建一个 DropPath 层（随机深度）。如果 drop_path 比率大于0，它会在训练时随机“丢弃”整个残差连接；否则，它就是一个 nn.Identity() 层，相当于什么都不做。这是一种防止过拟合的正则化技术。
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()           #实现随机深度，一种正则化技术。


        #前向传播函数，接收 Stem 模块的输出。x 在这里就是 inner_tokens。
    def forward(self, x, outer_tokens, H_out, W_out, H_in, W_in, relative_pos=None):
        B, N, C = outer_tokens.size()
        #首先对 outer_tokens 进行归一化 (outer_norm1)。然后送入 outer_attn (即 SS2D_Region 模块) 进行Mamba扫描，捕捉不同区域之间的全局依赖关系。最后通过残差连接 (+ outer_tokens) 将处理后的信息加回原始令牌。
        ...
        #local message:局部细化：这里的 x (即 inner_tokens) 首先被 reshape 成批次为 B*N 的序列。然后送入 inner_attn (即 SS2D_Region 模块) 进行Mamba扫描。
        # 因为批次是 B*N，所以这个扫描是在每个大区域内部独立进行的，用于细化每个区域内的局部特征。最后也通过残差连接将结果加回。
        ...
        # 关键融合步骤: x = x + out_atten 。这一行代码是融合的点睛之笔。它将经过全局交互后的
        # outer_tokens（包含了全局上下文信息）广播并叠加到其对应的inner_tokens上。这意味着，每个区域内部的局部特征都接收到了来自全局的、跨模态的引导信息。
        out_atten= outer_tokens.reshape(B*N, C).unsqueeze(1).repeat(1,self.kern*self.kern,1)
        x = x + out_atten
        #最后，通过一系列 reshape 和 permute 操作，将融合后的令牌重新组合成标准的 (B, H, W, C) 格式的2D特征图，并返回。
...
# 这是Region_global_Block内部使用的Mamba计算引擎 。它本质上是
# vmamba.py中SS2D模块的一个定制化版本，包含了完整的状态空间模型（SSM）计算流程，如输入投射、卷积、核心的selective_scan操作以及输出投射。
