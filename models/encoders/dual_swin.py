...
from .swinV2_T import SwinTransformerV2
...
class RGBXTransformer(nn.Module):
    def __init__(self,
                 ...
                 pretrained=None,  # 预训练权重路径会由 vssm_tiny_swinv2 等函数传入
                 out_indices=(0, 1, 2, 3),  # <--- 确认传入 out_indices
                 **kwargs):
        super().__init__()
        ...
        self.swin = SwinTransformerV2(
            ...
            out_indices=out_indices,  # <--- 确保传递
            pretrained_window_sizes=[0, 0, 0, 0]
        )
        # 加载预训练权重 (现在由 SwinTransformerV2 内部处理)
        if pretrained:
            self.swin.load_pretrained(pretrained)  # 调用 SwinV2 的加载方法
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        outs_rgb = self.swin(x_rgb) # B x C x H x W
        outs_x = self.swin(x_e) # B x C x H x W
...
# --- 便捷的创建函数 (保持不变或略作修改) ---
# 将类名改为 vssm_tiny_swinv2 可能更清晰
class swin_tiny(RGBXTransformer):
    def __init__(self, fuse_cfg=None, **kwargs):
        # 传递预训练路径到父类
        pretrained_path = 'pretrained/swinv2_tiny_patch4_window8_256.pth'  # <--- 您的 SwinV1 权重路径
        # 注意：这里需要 SwinV2 的权重！您需要下载 SwinV2-T 的权重文件
        # 例如：'path/to/swinv2_tiny_patch4_window8_256.pth' (文件名可能不同)
        # pretrained_path = 'path/to/swinv2_tiny_patch4_window8_256.pth' # <--- 应该用这个

        super(swin_tiny, self).__init__(
            img_size=512, patch_size=4, in_chans=3,
            embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
            window_size=8, drop_path_rate=0.1, ape=False, patch_norm=True,
            pretrained=pretrained_path,  # 传递路径
            out_indices=(0, 1, 2, 3),
            **kwargs
        )
